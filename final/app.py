...
def checkout():
    ...
    if request.method == 'POST':
        ...
        shipping_address = request.form.get('shipping_address')  # Get shipping address from form
        payment_method = request.form.get('payment_method', 'cash_on_delivery')  # Default payment method

        # Insert order into the database
        cursor.execute(
            "INSERT INTO orders (user_id, total_price, order_status, shipping_address, payment_method) VALUES (%s, %s, %s, %s, %s)",
            (user_id, total_price, 'pending', shipping_address, payment_method)
        )
